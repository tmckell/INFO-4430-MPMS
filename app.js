...
async function record_inventory(params) {
  ...
  if (!logged_in()) {
    show_home();
    return;
  } //in case followed a link after logging out. This prevents the user from using this feature when they are not authenticated.
  ...
  if (!params) {
    ...
    if (user_data.store.length === 1) {
      ...
      tag("inventory-message").innerHTML =
        '<i class="fas fa-spinner fa-pulse"></i>'; //this element is used to add a visual element (spinning wheel) to signify that the site is processing.
    ...
    if (response.status === "success") {
      ...
      tag("inventory-title").innerHTML = `<h2>${
        store_list()[params.store]
      } Ice Cream Inventory</h2>`;
      ...
      for (container of response.list.records[0].fields.container) {
        ...
        header.push(
          `<th onclick="hide_elements('col-${window.cols[container]}')" class="sticky col-${window.cols[container]}" >${cont}</th>`
        );
      }
      ...
      for (record of response.list.records) {
        ...
        for (container of record.fields.container) {
          target.push(
            `<td class="active col-${window.cols[container]}"><input id="${
              record.id
            }|${container.replace(/\s/g, "_")}" data-store="${
              params.store
            }" data-item_id="${
              record.id
            }" data-container="${container}" type="text" onchange="update_observation(this)"></td>`
          );
        }
      ...
      html.push(
        "<br>In this section, fill in only the rows corresponding to flavors you have on hand."
      );
      html.push(header.join(""));
      ...
      if (response.data.records) {
        for (record of response.data.records) {
          const box = tag(
            record.fields.item[0] +
              "|" +
              record.fields.container.replace(/\s/g, "_")
          );
          ...
          for (const div of getAllSiblings(box)) {
            ...
            if (
              div.tagName === "DIV" &&
              record.fields.quantity === val_map[div.innerHTML]
            ) {
      ...
      tag("inventory_panel").innerHTML =
        "Unable to get inventory list: " + response.message + ".";
    }
  }
}
    ...
    if (response.data.records) {
      ...
      for (const [key, value] of Object.entries(data)) {
        ...
        if (total_box.innerHTML === "") {
          ...
          total_box.innerHTML =
            parseFloat(total_box.innerHTML) + value.quantity;
        }
      }
    }
  } else {
...
function add_buttons(row, col) {
  ...
  switch (window.cols[col]) {
    case 3:
      ...
      container.appendChild(get_div_button(box, "20%", 0.25, "&#188;"));
      container.appendChild(get_div_button(box, "20%", 0.5, "&#189;"));
      container.appendChild(get_div_button(box, "20%", 0.75, "&#190;"));
...
function flavor_total(flavor_id) {
  ...
  for (const key of Object.keys(window.cols)) {
    if (isNaN(key)) {
      ...
      flavor_total +=
        parseFloat(tag(flavor_id + "|" + key.replace(/\s/g, "_")).value) || 0;
    }
  }
  return flavor_total;
}
...
async function update_observation(entry) {
  ...
  if (!logged_in()) {
    show_home();
    return;
  } //If the user logs out, not updates are permitted.
  ...
  if (entry.dataset.obs_id) {
    ...
    if (response.status === "success") {
      //if the value is successfully updated, the appearance of the cell is changed to reflect the update.
      ...
      //if the value is not successfully updated, the appearance of the cell is changed to reflect an error and an error message is presented to the user.
    ...
    if (response.status === "success") {
      //If it is inserted correctly, the appearance of the cell is changed to reflect the update.
      tag(flavor_id + "|total").innerHTML = flavor_total(flavor_id);
      entry.parentElement.classList.remove("working");
      entry.parentElement.classList.remove("active");
      entry.parentElement.classList.add("inactive");
      ...
      //If it is not inserted correctly, the appearance of the cell is changed to reflect the error and the error message is presented.
      entry.style.backgroundColor = "red";
      message({
...
async function show_project_list() {
  ...
  if (response.status === "success") {
    ...
    const header = [
      `
        <table class="inventory-table">
            ...
            <th class="sticky">Project Details</th>
            <th class="sticky">Project Type</th>
            <th class="sticky">Business</th>
            <th class="sticky">Shipment</th>
    ...
    for (const record of response.records) {
      ...
      html.push(`<td title="${record.fields.ProjectDetails}" class="ProjDet">${record.fields.ProjectDetails}</td>`);
      html.push(`<td>${record.fields.ProjectType}</td>`);
      html.push(`<td>${record.fields.BusinessName}</td>`);
      html.push(`<td>${record.fields.TrackingNumber}</td>`);
      html.push(`<td>${record.fields.ProductName}</td>`);
      html.push(`<td>${record.fields.CustomerName}</td>`);
      if (record.fields.Done) {
        html.push(`<td class="check-mark">&#x2714;</td>`);
      } else {
        html.push(`<td class="x-mark">&#x2718;</td>`);
      }
      html.push(
        `<td><button class="mark-as-complete" onclick="record_task('${record.id}')">${record.fields.Done ? 'Mark Incomplete' : 'Mark Complete'}</button></td>`
      );
      
      html.push("</tr>");
    }
    html.push(`
    <input type="button" class="show" style="float: right; margin-bottom: -1%" value="Add a new project" id="Show_Add_Inputs" onclick="show_add_project()">
    <br>
    <input type="button" class="hidden" value="Hide new project options" id="hide_Add_Inputs" onclick="hide_add_project()">
    <br>
    <div class="hidden" id="Add_Project_textInputs">
    <div style="background-color: #f9f9f9; padding: 20px;">
        <form>
            <label>Project Name: </label>
            <input type="text" placeholder="Project Name" id="projectName"><br><br>
            <label>Project Details: </label>
            <textarea placeholder="Project Details" id="projectDetails"></textarea><br><br>
            ...
            <input type="text" placeholder="Business Name" id="businessName"><br><br>
            <label>Shipment Number: </label>
            <input type="number" placeholder="Shipment Number" id="shipmentNumber"><br><br>
            <label>Product(s) Needed: </label>
            <textarea placeholder="Product(s) Needed:" id="projectProducts"></textarea><br><br>
            <label>Customers Name: </label>
            <input type="text" placeholder="Customers First Name" id="customerFName">
            <input type="text" placeholder="Customers Last Name" id="customerLName"><br><br>
            <label>Check if project is done: </label>
            <input type="checkbox" id="projectDone"><br><br>
        </form>
        </div>
    </div>
    <input type="button" class="hidden" value="Add project" id="Add_Project" onclick="add_project()">
`);
...
async function add_project() {
  ...
  var customerFVal = document.getElementById("customerFName").value;
  var customerLVal = document.getElementById("customerLName").value;
  var doneVal = document.getElementById("projectDone").checked;
  const response = await server_request({
    mode: "add_new_project",
    name: nameVal,
    projectDetails: detailsVal,
    type: typeVal,
    business: businessVal,
    shipment: shipmentVal,
    products: prudctsVal,
    customerF: customerFVal,
    customerL: customerLVal,
    done: doneVal,
  });
  show_project_list();
}
